    ...
    <main className="bg-[#0a0a0a] text-white font-sans overflow-x-hidden">
      <section ref={heroRef} className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <motion.div style={{ opacity }} className="relative z-10 text-center px-6 max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex items-center justify-center gap-3 mb-6">
            <div className="h-px w-12 bg-amber-400" />
            <span className="text-amber-400 text-xs tracking-[0.4em] uppercase font-medium">Est. 2009</span>
            <div className="h-px w-12 bg-amber-400" />
          </motion.div>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}
            ...
            Premium residential and commercial developments across India — crafted with precision, delivered with trust.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }} className="flex gap-4 justify-center flex-wrap">
            <Link href="/projects" className="group relative px-8 py-4 bg-amber-400 text-black font-semibold rounded-full overflow-hidden transition-all hover:shadow-[0_0_30px_rgba(251,191,36,0.4)]">
            ...
            <Link href="/contact" className="px-8 py-4 border border-white/30 text-white rounded-full hover:border-amber-400 hover:text-amber-400 transition-all backdrop-blur-sm">
              Contact Us
            </Link>
          </motion.div>
        </motion.div>
        ...
        <div className="max-w-5xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8">
          {STATS.map((s, i) => (
            <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center">
              <p className="text-4xl font-bold text-amber-400 mb-1" style={{ fontFamily: "'Georgia', serif" }}>{s.value}</p>
      ...
      <section className="py-24 px-6 max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
        <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
          ...
          <h2 className="text-4xl font-bold leading-tight mb-6" style={{ fontFamily: "'Georgia', serif" }}>Crafting spaces people are proud to call home</h2>
          <p className="text-stone-400 leading-relaxed mb-6">For over 15 years, Pavani Infra has been delivering premium developments across Hyderabad and beyond. Every project is a commitment to quality, transparency, and timeless design.</p>
          <Link href="/about" className="text-amber-400 text-sm hover:underline underline-offset-4">Our Story →</Link>
        </motion.div>
        <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative h-80 rounded-2xl overflow-hidden">
      ...
      <section className="py-20 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {featured.length === 0 ? (
              [
                { img: "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?w=600&q=80", title: "Sky Heights", loc: "Hyderabad", cat: "Ongoing" },
                { img: "https://images.unsplash.com/photo-1570129477492-45c003edd2be?w=600&q=80", title: "Green Valley", loc: "Bangalore", cat: "Completed" },
                { img: "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=600&q=80", title: "Pearl Residency", loc: "Hyderabad", cat: "Upcoming" },
              ].map((p, i) => (
              ...
              featured.map((p, i) => (
                <motion.div key={p._id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }}>
      ...
      <section className="py-24 px-6 relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative max-w-3xl mx-auto text-center">
          <p className="text-amber-400 text-xs tracking-[0.3em] uppercase mb-4">Get Started</p>
          <h2 className="text-5xl font-bold mb-6" style={{ fontFamily: "'Georgia', serif" }}>Find your perfect home today</h2>
          <p className="text-stone-400 text-lg mb-10 max-w-xl mx-auto">Our team is ready to help you discover the right property for your family or investment.</p>
          <Link href="/contact" className="inline-block px-12 py-4 bg-amber-400 text-black font-semibold rounded-full hover:bg-amber-300 hover:shadow-[0_0_40px_rgba(251,191,36,0.5)] transition-all text-lg">
            Schedule a Visit
          </Link>
        </motion.div>
      </section>
      ...
      <footer className="border-t border-white/5 py-10 px-6">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-stone-500 text-sm">© 2024 Pavani Infra. All rights reserved.</p>
